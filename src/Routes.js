...
const AppRoutes = () => {
    ...
    const list = [
        ...
        useEffect(() => {
            ...
            const summedOutflow = list.reduce((acc, item) => acc + item.saida, 0);
            setTotalOutflows(summedOutflow)
    ...
    return(
        <Router>
            <Navbar />
            <Routes>
                <Route path="/" element={<Home totalEntries={totalEntries} totalOutflows={totalOutflows} total={total} />}></Route>
                <Route path="/transactions" element={<Transactions list={list}/>}></Route>
            </Routes>
            <FooterDev />
        </Router>
    )
